...
extern "C"
{
    ...
    typedef struct
    {
        lcd_clock_source_t clk_src : LCD_CLK_SRC_DEFAULT; /*!< Clock source for the RGB LCD peripheral */
        esp_lcd_rgb_timing_t timings;                     /*!< RGB timing parameters, including the screen resolution */
        size_t data_width : 16;                           /*!< Number of data lines */
        size_t bits_per_pixel : 1;                        /*!< Frame buffer color depth, in bpp, specially, if set to zero, it will default to `data_width`.
                                                            When using a Serial RGB interface, this value could be different from `data_width` */
        size_t num_fbs : 1;                               /*!< Number of screen-sized frame buffers that allocated by the driver. By default (set to either 0 or 1) only one frame buffer will be used. Maximum number of buffers are 3 */
        size_t bounce_buffer_size_px : 1;                 /*!< If it's non-zero, the driver allocates two DRAM bounce buffers for DMA use.
                                                            DMA fetching from DRAM bounce buffer is much faster than PSRAM frame buffer. */
        size_t sram_trans_align : 32;                     /*!< Alignment of buffers (frame buffer or bounce buffer) that allocated in SRAM */
        size_t psram_trans_align : 32;                    /*!< Alignment of buffers (frame buffer) that allocated in PSRAM */
        int hsync_gpio_num : 1;                           /*!< GPIO used for HSYNC signal */
        int vsync_gpio_num : 1;                           /*!< GPIO used for VSYNC signal */
        int de_gpio_num : 1;                              /*!< GPIO used for DE signal, set to -1 if it's not used */
        int pclk_gpio_num;                                // : 1;                            /*!< GPIO used for PCLK signal */
        int disp_gpio_num : 1;                            /*!< GPIO used for display control signal, set to -1 if it's not used */
