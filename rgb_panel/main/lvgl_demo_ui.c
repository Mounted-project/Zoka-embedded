...
//     NULL); lv_obj_set_style_line_width(chart, 0, LV_PART_ITEMS); /*Remove the
//     lines*/

//     lv_chart_set_type(chart, LV_CHART_TYPE_SCATTER);

//     lv_chart_set_axis_tick(chart, LV_CHART_AXIS_PRIMARY_X, 5, 5, 5, 1, true,
//     30); lv_chart_set_axis_tick(chart, LV_CHART_AXIS_PRIMARY_Y, 10, 5, 6, 5,
//     true, 50);
...
//     lv_palette_main(LV_PALETTE_RED), LV_CHART_AXIS_PRIMARY_Y); for (int i =
//     0; i < 50; i++)
//     {
//         lv_chart_set_next_value2(chart, ser, lv_rand(0, 200), lv_rand(0,
//         1000));
//     }
...
void example_lvgl_demo_ui(lv_disp_t *disp) {
  ...
  // Create a square of 200x200 pixels
  lv_obj_t *square = lv_obj_create(scr);
  lv_obj_set_size(square, 200, 200); // Set the size of the square
  lv_obj_align(square, LV_ALIGN_CENTER, 0,
               0); // Center the square on the screen

  // Define the styles for the square
  static lv_style_t style_square;
  lv_style_init(&style_square);

  // Set background color (black)
  lv_style_set_bg_color(&style_square, lv_color_black());
  // Set border color (white)
  lv_style_set_border_color(&style_square, lv_color_white());
  // Set border width
  lv_style_set_border_width(&style_square, 2);

  // Apply the style to the square
  lv_obj_add_style(square, &style_square, 0);
}
