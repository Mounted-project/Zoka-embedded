...
void ui_object_set_themeable_style_property(
  ...
  ui_object_set_local_style_property(object_p, selector, property,
                                     ui_get_theme_value(theme_variable_p));
}
...
lv_style_value_t _ui_style_value_convert(lv_style_prop_t property,
  ...
  if (property == LV_STYLE_BG_COLOR || property == LV_STYLE_BG_GRAD_COLOR ||
      ...
      property == LV_STYLE_SHADOW_COLOR || property == LV_STYLE_IMG_RECOLOR ||
      property == LV_STYLE_LINE_COLOR || property == LV_STYLE_ARC_COLOR ||
      property == LV_STYLE_TEXT_COLOR) {
    Style_Value.color = lv_color_hex(value);
  } else if (property == LV_STYLE_BG_GRAD || property == LV_STYLE_BG_IMG_SRC ||
...
_ui_local_style_create(const ui_style_variable_t *style_variable_p,
  ...
  _ui_local_styles =
      lv_mem_realloc(_ui_local_styles,
...
// auto-update dynamic local styre-array's 2nd dimension (object part+state
...
_ui_local_style_property_setting_t *_ui_local_style_property_setting_create(
  ...
  for (i = 0; i < local_style_p->style_property_setting_count; ++i) {
    ...
    if (FoundEmptyPosition < 0) { // search and register one empty position that
                                  // might be needed for a newly created entry
      if (style_property_setting_p->object_p == NULL)
        FoundEmptyPosition = i; // if found empty position, register it
      else if (!lv_obj_is_valid(style_property_setting_p
                                    ->object_p)) { // if not, free up an entry
                                                   // of a deleted widget
  ...
  if (FoundEmptyPosition < 0) {
    local_style_p->style_property_settings =
        lv_mem_realloc(local_style_p->style_property_settings,
                       (local_style_p->style_property_setting_count + 1) *
    ...
    style_property_setting_p =
        &local_style_p->style_property_settings[FoundEmptyPosition];
  }
